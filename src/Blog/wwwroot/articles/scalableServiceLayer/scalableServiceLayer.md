...
AdaptiveClient is a utility that provides a pattern for using Entity Framework Core and Autofac together.  AdaptiveClient eliminates much of the redundant work related to registering and resolving services. It also provides utilities for working with specific Entity Framework Core objects.  Most importantly, however, AdaptiveClient allows the developer to create granular, loosely coupled services that are resolved at runtime using keys (an explanation and an example of using keys to register and resolve services is given later in this article).  

Registering services using keys has the benefit of allowing the components of the application to be very loosely coupled.  Furthermore, it assigns responsibility for assembling components at runtime to the dependency injection container, in this case, Autofac.  Now you might be thinking that registering and resolving dependencies using keys will quickly become a nightmare to manage.  AdaptiveClient addresses those concerns by providing a top-down pattern for defining keys and a simple API that centralizes and streamlines the registration and resolution of dependencies. 
